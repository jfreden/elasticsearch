...
public class SecurityIndexBoolQueryBuilderTests extends ESTestCase {
    ...
    public void testBuildFromBoolQuery() {
        ...
        assertThat(apiKeyQb1.filter(), hasItem(QueryBuilders.termQuery("type", "api_key")));
    ...
    public void testWillFilterForApiKeyId() {
        ...
        assertThat(apiKeyQb.filter(), hasItem(QueryBuilders.termQuery("type", "api_key")));
    ...
    private void testAllowedIndexFieldName(Predicate<String> predicate) {
        final String allowedField = randomFrom(
            ...
            // "creator." + randomAlphaOfLengthBetween(1, 10) TODO Don't think this is a valid test!?
            "creator.realm",
            "creator.principal"
        );
    ...
    private void assertCommonFilterQueries(SecurityIndexBoolQueryBuilder qb, Authentication authentication) {
        final List<TermQueryBuilder> tqb = qb.filter()
            ...
            .toList();
        assertTrue(tqb.stream().anyMatch(q -> q.equals(QueryBuilders.termQuery("type", "api_key"))));
